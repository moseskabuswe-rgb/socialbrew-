...
function CommentsSection({ ratingId, onClose }: { ratingId: string; onClose: () => void }) {
  ...
  async function postComment() {
    ...
    if (data) {
      ...
      // Update comment count on rating
      ...
      <div className="w-full max-w-sm bg-white rounded-t-3xl animate-slide-up flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
          ...
          {!loading && comments.length === 0 && (
            <p className="text-center text-coffee-400 text-sm py-8">No comments yet. Be the first!</p>
          )}
        ...
        <div className="px-4 py-3 border-t border-cream-200 flex gap-2">
          <input
            ...
            className="flex-1 bg-cream-100 rounded-full px-4 py-2 text-sm text-coffee-800 placeholder-coffee-400 focus:outline-none border border-cream-200"
          />
          <button onClick={postComment} disabled={!newComment.trim() || posting}
            className="bg-caramel text-white rounded-full px-4 py-2 text-sm font-semibold disabled:opacity-40 transition-all">
            {posting ? '...' : 'Post'}
          </button>
        </div>
      </div>
...
export default function HomeTab({ refresh }: { refresh: number }) {
  ...
  function toggleSave(ratingId: string) {
    setSavedIds(prev => {
      const n = new Set(prev)
      n.has(ratingId) ? n.delete(ratingId) : n.add(ratingId)
      return n
    })
  }
        ...
        {ratings.map(rating => {
          ...
          return (
            <div key={rating.id} className="bg-white mx-4 mb-3 rounded-2xl shadow-sm border border-cream-200 overflow-hidden animate-fade-in">
              ...
              {/* Shop card */}
              {shop && (
                <button onClick={() => setSelectedShop(shop)} className="mx-4 mb-3 flex items-center gap-3 bg-cream-100 rounded-xl p-2.5 border border-cream-200 w-full text-left hover:bg-cream-200 transition-colors">
              ...
              <div className="flex items-center px-4 pb-4 gap-4">
                ...
                <button className="ml-auto text-coffee-400 flex items-center gap-1.5 opacity-50">
                  <Gift size={18} />
                  <span className="text-xs">Gift</span>
                </button>

                <button onClick={() => toggleSave(rating.id)} className="transition-all active:scale-90"
