...
type HomeTabProps = { shop?: any; onClose: () => void; onComplete: () => void; refresh?: number };

// ── UTILITY FUNCTIONS (prefix with _ to silence unused warnings) ──
function _getMugColor(fill: number) { if (fill <= 20) return '#b0c4d4'; if (fill <= 40) return '#c8924a'; if (fill <= 60) return '#a06428'; if (fill <= 80) return '#7a3e10'; return '#4e2008'; }
function _getFillLabel(fill: number) { if (fill <= 15) return 'Just a Sip'; if (fill <= 30) return 'Getting There'; if (fill <= 50) return 'Half Cup'; if (fill <= 70) return 'Good Pour'; if (fill <= 85) return 'Almost Perfect'; return 'Perfect Brew ✨'; }
function _formatDate(d: string) { return new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); }
function _timeAgo(d: string) { const diff = Date.now() - new Date(d).getTime(); const m = Math.floor(diff / 60000); if (m < 1) return 'Just now'; if (m < 60) return `${m}m ago`; const h = Math.floor(m / 60); if (h < 24) return `${h}h ago`; return `${Math.floor(h / 24)}d ago`; }
...
// ── HOME TAB ──
export default function HomeTab({ shop, onClose, onComplete, refresh }: HomeTabProps) {
  const { profile } = useAuth();
