        ...
        {activeTab === 'profile' && (
          <>
            ...
            {!loading && ratings.length > 0 && (() => {
              ...
              const topShop = visitedShops[0]?.coffee_shops
              // Favorite drink by frequency
              const drinkCounts: Record<string, number> = {}
              ratings.forEach(r => { if (r.drink_name) drinkCounts[r.drink_name] = (drinkCounts[r.drink_name] || 0) + 1 })
              const favDrink = Object.entries(drinkCounts).sort((a, b) => b[1] - a[1])[0]?.[0]
              // This month's visits
              ...
              return (
                <div className="mx-4 mt-3 grid grid-cols-3 gap-2">
                  {topShop && (
                    <div className="bg-white rounded-2xl p-3 border border-cream-200 shadow-sm text-center">
                      <p className="text-coffee-400 text-xs mb-1">Top Shop</p>
                      <p className="text-coffee-800 font-bold text-xs leading-tight line-clamp-2">{topShop.name}</p>
                    </div>
                  )}
                  {favDrink && (
                    <div className="bg-white rounded-2xl p-3 border border-cream-200 shadow-sm text-center">
                      <p className="text-coffee-400 text-xs mb-1">Fav Drink</p>
                      <p className="text-coffee-800 font-bold text-xs leading-tight line-clamp-2">{favDrink}</p>
                    </div>
                  )}
                  <div className="bg-white rounded-2xl p-3 border border-cream-200 shadow-sm text-center">
                    <p className="text-coffee-400 text-xs mb-1">This Month</p>
                    <p className="text-coffee-800 font-bold text-sm">{thisMonth} sip{thisMonth !== 1 ? 's' : ''}</p>
                  </div>
                </div>
              )
            })()}
