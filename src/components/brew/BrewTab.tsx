...
type BrewAction = 'rate' | 'share' | 'gift' | 'order' | 'quicksip'
...
export default function BrewTab({ onPostCreated }: Props) {
  ...
  function handleAction(a: BrewAction) {
    ...
    if (a === 'quicksip') { setAction('quicksip'); return }
    setAction(a)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-tan-50">
      ...
      {action === 'quicksip' && <QuickSip onClose={handleClose} onComplete={onPostCreated} />}
    </div>
  )
}
