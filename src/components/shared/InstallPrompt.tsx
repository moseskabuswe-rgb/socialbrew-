...
export default function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      // Analytics hook — fires when install prompt is available
      console.log('[SocialBrew] Install prompt available (Android/Chrome)')
    ...
    if (isIOS() && isSafari()) {
      ...
      }, 2500) // slight delay so it doesn't fire on first load
      return () => {
        clearTimeout(timer)
    ...
    window.addEventListener('appinstalled', () => {
      console.log('[SocialBrew] App successfully installed!')
  ...
  function trackInstallEvent(action: string, platform: string) {
    // Console log for now — swap this for your analytics provider later
    console.log(`[SocialBrew Analytics] install_${action} | platform: ${platform} | time: ${new Date().toISOString()}`)
    // Example Posthog call when you're ready:
  ...
  if (showIOS) {
    return (
      <>
        ...
        >
          ...
          {[
            ...
            },
            {
              ...
              text: 'Tap',
              highlight: '"Add"',
              suffix: 'in the top right — you\'re in.',
            },
          ].map((step) => (
            <div
              key={step.num}
            ...
            >
              <div style={{
                ...
                background: 'rgba(200,133,58,0.2)',
                border: '1px solid rgba(200,133,58,0.4)',
              ...
              <p style={{ color: 'rgba(245,230,200,0.8)', fontSize: 14, margin: 0, lineHeight: 1.5, fontFamily: 'system-ui, sans-serif' }}>
                {step.text}{' '}
                <span style={{ color: '#c8853a', fontWeight: 700 }}>{step.highlight}</span>
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(245,230,200,0.6)', fontSize: 12, margin: 0, fontFamily: 'system-ui, sans-serif' }}>
              Look for the share icon at the bottom of your screen — it looks like a box with an arrow pointing up.
            </p>
          </div>
        ...
        {/* Arrow pointing down toward actual Safari share bar */}
        <div style={{
          position: 'fixed',
          bottom: 0,
          left: '50%',
